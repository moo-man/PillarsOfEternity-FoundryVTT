...
export default class AgingDialog extends Dialog 
{
    ...
    static async create({modifier, changeList, changes, years, defaultYear} : AgingDialogData["dialogData"]) 
    {
        ...
        const html = await renderTemplate("systems/pillars-of-eternity/templates/apps/aging-dialog.hbs", {modifier, changeList, changes, years, defaultYear: defaultYear || years[0]});
            ...
            return new this({
                ...
                buttons : {
                    roll : {
                        ...
                        callback : (dlg) : void => 
                        {
                            const data : AgingCheckDataFlattened = <AgingCheckDataFlattened>{};
                            const html = $(dlg);
    ...
    _onEffectSelect(ev : JQuery.ChangeEvent) 
    {
        ...
        {
            const input = this.dynamicInputs[change.key as keyof typeof this.dynamicInputs]?.[0] as HTMLInputElement;
            if (input) 
            {
