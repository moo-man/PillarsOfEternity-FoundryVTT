...
export default class PillarsCombatTracker extends CombatTracker 
{
    ...
    {
        const combat = this.viewed as PillarsCombat;
        ...
        if (combat?.round && combat.round > 0) 
        {
            ...
            {
                const combatant = combat.combatants.get(element.dataset.combatantId as string) as PillarsCombatant;
                const move = combatant.getMoveData();
                if (!move) {return;}
