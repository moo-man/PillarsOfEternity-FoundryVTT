...
export class PillarsItemSheet extends ItemSheet {
  ...
  async _onDrop(ev) {
    ...
    if (itemData && itemData.type === "power" && game.pillars.config.allowEmbeddedPowers.includes(this.item.type)) {
  ...
  async handleEmbeddedPowerDrop(power) {
    ...
    if (this.item.type == "equipment" && this.item.category.value == "grimoire")
      power.data.embedded.spendType = "source"
    ...
    {
      ownedPower = (await this.actor.createEmbeddedDocuments("Item", [power]))[0]
      power.ownedId = ownedPower.id
    }
  ...
  handleSummonedItem(itemData)
  {
    let summons = duplicate(this.item.summons)
  ...
  activateListeners(html) {
    ...
    html.find(".add-property").click(ev => {
      ...
      if (property == "summons")
        return ui.notifications.notify("Drag and drop an Item to create a Summon")
    ...
    html.find(".summon a").click(ev => {
      let property = $(ev.currentTarget).parents(".form-group").attr("data-property")
      let index = $(ev.currentTarget).parents(".property-inputs").attr("data-index")
      let array = foundry.utils.deepClone(getProperty(this.item, property))
      let summon = array[index];
      new PillarsItem(summon.data).sheet.render(true, {editable: false})
    })
