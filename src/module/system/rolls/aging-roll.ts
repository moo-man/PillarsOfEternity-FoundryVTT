...
export default class AgingRoll 
{
    ...
    async rollCheck() 
    {
        const lifestyleMod = PILLARS.lifestyleModifier[this.checkData?.lifestyle as keyof typeof PILLARS.lifestyleModifier];
        ...
        const message = this.handleAgeRollResult();
    ...
    handleAgeRollResult() 
    {
        ...
        try 
        {
            if (this.roll && this.roll.total)
            {
                ...
                {
                    const points = this._pointsToNextDeathMarch(2);
        ...
        {
            const error = game.i18n.localize("PILLARS.ErrorComputeAgingRoll") + ": " + e;
    ...
    _pointsToNextDeathMarch(marches = 0) : number
    {
        ...
        {
            const march = this.actor.system.life.march;
            let index = -1;
            if (march > 0)
            {index = march - 1;} 

            const pointThresholds = Object.keys(PILLARS.agePointsDeathRank).map(i => Number(i));

            const targetThreshold = march + marches;

            const agingPoints = this.actor.system.life.agingPoints;
