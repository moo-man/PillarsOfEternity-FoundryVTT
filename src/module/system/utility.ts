...
export class PILLARS_UTILITY 
{
    ...
    static log(message : string, force  = false, args : unknown) 
    {
    ...
    static async findSkillName(name: string, actor?: PillarsActor): Promise<PillarsItem | undefined> 
    {
        ...
        if (actor) 
        {
            skill = actor.getItemTypes(ItemType.skill).find((i) => i.name == name);
        ...
        for (const pack of game.packs) 
        {
            ...
            const skillIndex = pack.index.find((i) => i.name == name);
            if (skillIndex) 
            {
