...
export class PillarsItem extends Item 
{
    ...
    async _preCreate(data: ItemDataConstructorData, options: DocumentModificationOptions, user: User) 
    {
        ...
        {
            const item = this.actor!.items.find((i) => i.type == this.type && i.id != this.id);
    ...
    async _preDelete(options: DocumentModificationOptions, user: User) 
    {
        ...
        {
            const embeddedPowers = this.actor!.getItemTypes(ItemType.power).filter((i) => i.system.embedded?.item == this.id);
    ...
    _onUpdate(data: ItemDataSource, options: DocumentModificationOptions, user: string) 
    {
        ...
        const embeddedParent = this.EmbeddedPowerParent;
    ...
    async checkBondTraitEffects() 
    {
        if(this.data.type == "bond" && this.isOwned)
        {
            ...
            const toDelete = [];
            const bondEffects = this.actor!.effects.filter(i => !!(i.getFlag("pillars-of-eternity", "bondTrait") as string));

            // Find traits owned but with no effect and add them
            for(const trait of this.system.traits)
            {
                const existing = bondEffects.find(i => trait == i.getFlag("pillars-of-eternity", "bondTrait") as string);
                if (!existing)
            ...
            {
                const key = effect.getFlag("pillars-of-eternity", "bondTrait") as string;
    ...
    prepareData() 
    {
        ...
        const prepareFunction = (this[`prepare${this.type[0]!.toUpperCase() + this.type.slice(1)}` as keyof this]) as unknown as () => unknown;
    ...
    prepareOwnedData() 
    {
        const prepareOwnedFunction = (this[`prepareOwned${this.type[0]!.toUpperCase() + this.type.slice(1)}` as keyof this]) as unknown as () => unknown;
    ...
    prepareWeapon() 
    {
        ...
        const category = this.system.category?.value || "";
    ...
    async postToChat() 
    {
        const chatData = <ItemChatData>this.dropdownData();

        chatData.item = this;

        const html = await renderTemplate("systems/pillars-of-eternity/templates/chat/post-item.hbs", chatData);
        const cardData = <ChatMessageDataConstructorData>{ content: html };
    ...
    preparePowerGroups(): PowerGroups 
    {
        ...
        try 
        {
            const config = getGame().pillars.config;
            let groupIds: string[] = [];
    ...
    {
        const targetSubTypes = getGame().pillars.config[`power${target.value[0]?.toUpperCase() + target.value.slice(1)}s` as keyof typeof PILLARS];
    ...
    calculatePowerLevel(): number 
    {
        ...
        if (this.data.type == "power") 
        {
            ...
            {
                let pl = 0;
                const values = getGame().pillars.config.powerLevelValues;

                for (const range of this.system.range!) {pl += values.powerRanges[range.value as keyof typeof PILLARS.powerRanges] || 0;}

                for (const target of this.system.target!) 
                {
                    const targetSubTypes = values[`power${target.value[0]?.toUpperCase() + target.value.slice(1)}s` as keyof typeof values];
    ...
    {
        const powers = duplicate(this.system.powers || []);
        if (powers[index]) 
        {
    ...
    async _checkGrimoirePowers() 
    {
        ...
        const game = getGame();
        const nonArcanaPowers = powers.filter((i : EmbeddedPower) => i.system.source.value != "arcana");
                ...
                new Dialog({
                    ...
                    buttons: {
                        remove: {
                            ...
                            callback: async () => 
                            {
                                const arcanaPowers = powers.filter((i : EmbeddedPower) => i.system.source.value == "arcana");
    ...
    get Target() 
    {
        try 
        {
            const targetObj = this.system.target?.find((i : PowerTarget) => (i.group || getGame().i18n.localize("Default")) == this.displayGroupKey("target"));
            if (!targetObj) {return;}
            const targetSubTypes = getGame().pillars.config[`power${targetObj!.value[0]!.toUpperCase() + targetObj.value.slice(1)}s` as keyof typeof PILLARS];
    ...
    get Specials() 
    {
        const specials = this.specialList;
        const notSkilledEnough = this.system.special?.value.filter((i : WeaponSpecial) => this.isOwned && specials[i.name as keyof typeof specials]?.skilled && (!this.Skill || (this.Skill?.rank || 0) < 5));
    ...
    get specials() 
    {
        const specials: { [key: string]: WeaponSpecialData } = {};
